...
class EtlCrawl(threading.Thread):
    ...
    def __init__(self, queue, ubboard_id, board_name):
        ...
        login_form = {
            'si_redirect_address':'https://sso.snu.ac.kr/snu/ssologin_proc.jsp?si_redirect_address=http://my.snu.ac.kr/mysnu/login?langKnd=ko&loginType=portal',
            'si_realm':'SnuUser1',
            '_enpass_login_':'submit',
            'langKnd':'ko',
            'si_id':username,
            'si_pwd':password,}
        login_form_req = self.s.post('https://sso.snu.ac.kr/safeidentity/modules/auth_idpwd', data=login_form)

        redirect_url = 'https://sso.snu.ac.kr/nls3/fcs'
        redirect_data = bs(login_form_req.text, 'html5lib').find_all('input')
        redirect_form = { redirect_datum['name'] : redirect_datum['value'] for redirect_datum in redirect_data }
    ...
    def init_crawl(self, ubboard_id):
        etl = self.s.get('http://etl.snu.ac.kr/mod/ubboard/view.php?id=' + str(ubboard_id) + '&ls=1000&page=1')
